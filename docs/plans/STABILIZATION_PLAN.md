...
- 旧 `Data` 和 `Archive/migration` 已完成退役转移：活动位置均已移除，冷存储校验通过；退役总控实验已无业务 blocker，等待提交后做最终 clean run。
...
## Archive Studio 本地入口分离

状态：第一版完成，等待用户体验确认。

- [x] 从公开 `src/App.tsx` 移除 Studio imports、路由和导航入口。
- [x] 建立不进入默认生产构建的独立本地 Studio HTML / React 入口。
- [x] 用统一顶部导航替代五个页面重复标题、状态栏和板块 tabs。
- [x] 隐藏非必要解释副标题和成功统计，保留维护所需表单、素材、预览、预检、保存、同步与首页精选。
- [x] 建立后台启动脚本、快捷方式安装脚本和网站图标快捷方式。
- [x] 验证五个管理板块、桌面 / 390px 布局、公开路由隔离、API 回归和生产构建边界。

下一步只做用户体验确认与独立提交；不新增复杂条目类型、删除、批量编辑或自动发布。

## Archive Studio 高保真条目预览

状态：第一版完成，等待用户体验确认。

- [x] 明确公开站与本地 Studio 继续保持入口和构建隔离。
- [x] Games / Visions Studio 预览复用公开站实际海报卡。
- [x] Music Studio 默认展示实际当前专辑结构，放大层同时展示专辑库卡片。
- [x] Texts 根据条目类型展示实际普通条目卡或书架卡。
- [x] 支持本地临时素材地址、已有条目公开缩略图和轻量放大查看。
- [x] 完成 TypeScript、四板块更新事务、公开页面回归及桌面 / 390px 浏览器验收。

下一步只做用户体验确认与独立提交；若真实使用发现个别板块的预览密度不合适，只调整共享组件容器，不再新增模拟卡片。

## Archive Studio 紧凑工作台

状态：第一版完成，等待用户体验确认。

- [x] 参考 `YuDesign` 统一 Studio 的色彩、字体、细线分区和紧凑工具控件。
- [x] 四板块左侧表单改为桌面三列布局，缩短普通输入、备注、正文、素材和类型选择区域。
- [x] 桌面 1280 x 720 下四板块左侧编辑区完整可见。
- [x] 390px 窄屏恢复单列和 44px 触控高度，底部操作区压缩且无横向溢出。
- [x] TypeScript、Studio 更新事务和公开页面回归检查通过。
- [x] 合并手动 preview / preflight / save 为单次受控保存，并保留原安全门槛。
- [x] 新建成功后自动清空上一条目并生成新草稿 ID；新建 ID 可调整，修改 ID 继续锁定。
- [x] 四板块修改模式增加整条目删除，采用首页引用阻断、事务备份、一次性 token 和失败回退。
- [x] 已公开条目删除后通过现有显式同步从公开 JSON 移除，不自动发布。
- [x] 删除事务系统临时目录回归和桌面 / 窄屏确认窗口验收通过。

下一步只做用户体验确认和独立提交；不借本次视觉重设计改公开收藏展示、数据结构或写入流程。

## Archive Studio Games 赛季新建

状态：第一版完成，等待真实使用确认。

- [x] 在 Games 新建模式增加 `普通游戏 / 赛季` 轻量切换。
- [x] 只允许选择现有且已公开同步的 `live_game` 父条目。
- [x] 自动生成 season ID、建议下一顺序，并按父游戏现有结构显示扩展字段。
- [x] 建立 season preview、preflight、create、shape check、旧 Data 只读检查和失败回退。
- [x] 扩展公开同步：保持 live game 所在分组、公开 ID、已有赛季 ID 和媒体路径，只新增新赛季与优化后的封面。
- [x] 临时目录端到端测试和 1280 x 720 Studio 页面验收通过，未向真实 Archive 创建测试赛季。
- [x] 完成首次真实赛季同步，Archive / 公开 JSON 赛季数一致，新增派生封面存在。
- [x] 修复同步状态请求竞态与 OneDrive 云标记残留：最新请求优先，保存后复检，已同步标记不再重复入队。
- [x] 完成一次既有赛季封面受控替换：事务备份、结构检查、Games 媒体优化、公开同步和真实页面验收均通过。

下一步只做用户实际创建与同步体验确认；不同时开放 live game 父条目编辑、已有赛季修改/删除或 DLC。
...
轻量编辑第一版已完成，Games 已额外开放现有 `live_game` 下的新建 season。旧 `Data` 覆盖审计已建立并确认当前不具备退役条件；在确认 Archive 全量覆盖、旧生成/发布链路退役或加保护、外部备份可用前，不删除、不改名旧 `Data`，不清理 `Archive/migration`。DLC、live game 父条目编辑、已有 season 修改/删除、Visions showcase、批量编辑和自动发布仍不开放。
