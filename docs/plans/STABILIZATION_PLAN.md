...
状态：阶段 5 v2 Music live-compatible replacement 已完成并推送，Archive Studio v0 真实 v2 Music create smoke test 显式执行 gate 已建立。
...
- [ ] 实现真实 v2 Music create + rollback smoke test runner，默认停在计划模式。
...
1. 实现真实 v2 Music create + rollback smoke test runner，默认停在计划模式；
