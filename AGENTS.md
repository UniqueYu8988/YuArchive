...
`C:\Users\Yu\OneDrive\图片\Data` 是 YuArchive 的旧版收藏源数据、迁移来源和回退备份。AI 默认只能只读分析，不得自动修改、整理、迁移、导入、导出或清理其中任何内容。`Archive` 是当前新工作流的维护数据目录；写入它仍必须来自明确任务，并具备预览、校验和回退，不能使用临时脚本随意改写。
