...
function toRelative(filePath) {
  return path.relative(MIGRATION_ROOT, filePath).replace(/\\/g, '/');
}
...
export function buildArchiveMigrationColdStoragePlan({ now = new Date() } = {}) {
  ...
  const topLevelEntries = exists
    ? listDirSafe(MIGRATION_ROOT).map(entry => `${entry.isDirectory() ? 'dir' : 'file'}:${entry.name}`).sort()
    : [];
  const manifestDigest = crypto.createHash('sha256')
    ...
    .digest('hex');
  const targetLabel = `migration-${timestampLabel(now)}`;
  return {
    ok: exists,
    exists,
...
function printResult(result) {
  console.log(`[${result.ok ? 'PASS' : 'FAIL'}] Archive migration cold-storage dry-run`);
...
function main() {
  const result = buildArchiveMigrationColdStoragePlan();
  printResult(result);
