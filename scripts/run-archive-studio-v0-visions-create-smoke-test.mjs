...
const AUTHORIZATION_PHRASE = 'I authorize Archive Studio Visions create rollback smoke test';
const EXPECTED_BASELINE_ENTRIES = 112;
const EXPECTED_CHARACTERS = 20;
const POSTER_FIXTURE = path.resolve('public', 'icons', 'visions-character.webp');
...
function buildPayload() {
  ...
  return {
    ...
    board: 'visions',
    kind: 'movie',
    id: `vision-${day}-${crypto.randomUUID().replace(/-/g, '').slice(0, 8)}`,
    fields: {
      title: 'Archive Studio Visions smoke test',
      period: '此岸',
      cinema: false,
      quote: '',
      url: '',
    ...
    assets: {
      poster: {
        ...
        originalName: 'smoke-poster.webp',
        extension: '.webp',
      },
    },
  };
...
async function createForm(token, payload) {
  const poster = await readFile(POSTER_FIXTURE);
  const form = new FormData();
  form.set('payload', JSON.stringify(payload));
  form.set('preflightToken', token);
  form.set('poster', new Blob([poster], { type: 'image/webp' }), payload.assets.poster.originalName);
  return form;
}

async function rollbackCreatedVision({ result, v2Root, transactionsRootExisted, studioRootExisted }) {
  ...
  const kindDir = path.dirname(entryDir);
  ...
  await removeIfEmpty(kindDir);
}

export async function executeVisionsCreateRollbackSmokeTest({
  ...
  if (!baselineShape.ok || baselineShape.totalEntries !== EXPECTED_BASELINE_ENTRIES) {
    throw new Error('unexpected_visions_baseline');
  }
  ...
  const server = createArchiveStudioServer({
    ...
    sourceRoot,
    writeEnabled: true,
    expectedMinimumVisionsEntries: EXPECTED_BASELINE_ENTRIES,
    expectedVisionsCharacters: EXPECTED_CHARACTERS,
    requireVisionsMigrationBaseline: true,
  });
  ...
  try {
    ...
    if (preflight.response.status !== 200 || !preflight.body.preflightToken) {
      throw new Error('preflight_failed');
    }
    const create = await requestJson(baseUrl, '/api/studio/visions/create', {
      method: 'POST',
      body: await createForm(preflight.body.preflightToken, payload),
    });
    if (create.response.status !== 201 || !create.body.ok) throw new Error('create_failed');
    createResult = create.body;
    if (
      createResult.visionsEntries !== EXPECTED_BASELINE_ENTRIES + 1
      || createResult.check?.ok !== true
      || createResult.sourceUnchanged !== true
      || createResult.publishTriggered !== false
    ) throw new Error('post_create_invariants_failed');
  } finally {
    server.close();
    await once(server, 'close');
    if (createResult) {
      await rollbackCreatedVision({
        result: createResult,
        v2Root,
  ...
  const postRollbackShape = evaluateVisionsV2Shape({
    v2Root,
    expectedMinimumEntries: EXPECTED_BASELINE_ENTRIES,
    expectedCharacters: EXPECTED_CHARACTERS,
    requireMigrationBaseline: true,
  ...
  const transactionResidual = await exists(resolveInside(
    v2Root,
    `migration/archive-studio-v0/transactions/${createResult.transactionId}`,
  ));
  if (!postRollbackShape.ok || postRollbackShape.totalEntries !== EXPECTED_BASELINE_ENTRIES) {
    throw new Error('post_rollback_visions_shape_failed');
  }
  ...
  return {
    ...
    postWriteEntries: createResult.visionsEntries,
...
export async function run(argv = []) {
  ...
  console.log(`  entryResiduals: ${result.entryResiduals}`);
  console.log(`  transactionResiduals: ${result.transactionResiduals}`);
  ...
  console.log(`  publishTriggered: ${result.publishTriggered}`);
  console.log('  writeScope: single-temporary-vision-entry-rolled-back');
...
async function main() {
  try {
    ...
    console.log('[FAIL] Archive Studio Visions create + rollback smoke test');
