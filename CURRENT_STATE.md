...
ArchiveData-v2 的 Music v2 试点、live-compatible 数据替换和远端同步已完成，当前进入 Archive Studio v0 CLI sandbox preview 原型阶段。
...
- 当前工作区因 Archive Studio v0 preview core 模块拆分和状态文档更新而存在新的未提交变更。
...
只建议做一件事：验证并提交 Archive Studio v0 preview core 模块拆分；不要 push，不接 UI，不写真实 ArchiveData-v2 输出，不运行发布脚本，不运行 `build_archive.py`。
