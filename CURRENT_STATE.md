...
ArchiveData-v2 的 Music v2 试点、live-compatible 数据替换和远端同步已完成，当前已具备 Archive Studio v0 真实 v2 Music create + rollback smoke test runner 的默认计划模式。
...
- 当前工作区因 Archive Studio v0 real write create smoke test runner 和状态文档更新而存在新的未提交变更。
...
只建议做一件事：为真实 v2 Music create + rollback smoke test runner 增加真实执行实现；实现后仍必须默认计划模式，且只有用户在当前任务中明确授权、命令包含 `--execute` 和指定 entry id 时才允许真实写入。
