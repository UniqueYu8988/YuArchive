...
ArchiveData-v2 的 Music v2 试点、live-compatible 数据替换和远端同步已完成，当前已具备 Archive Studio v0 真实 v2 Music create smoke test 的只读计划模式和显式执行 gate 设计，下一步可实现默认计划模式的真实 smoke test runner。
...
- 当前工作区因 Archive Studio v0 real write create execute gate 文档和状态文档更新而存在新的未提交变更。
...
只建议做一件事：实现真实 v2 Music create + rollback smoke test runner，但默认停在计划模式；只有用户在当前任务中明确授权且命令包含 `--execute` 和指定 entry id 时，才允许真实写入。
